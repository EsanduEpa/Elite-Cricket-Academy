// Generic Profile Image Upload Functionality
// Can be used by any user role (Player, Admin, Coach, Trainer, Shop)
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Get the controller name from the current URL (player, admin, coach, etc.)
    const controllerName = window.location.pathname.split('/')[2] || 'player';
    ...
    function uploadProfileImage(file) {
        ...
        // Send AJAX request to the appropriate controller
        fetch(window.location.origin + '/Elite/' + controllerName + '/uploadProfileImage', {
            method: 'POST',
            body: formData
        })
    ...
    if (deleteImageBtn) {
        deleteImageBtn.addEventListener('click', function(e) {
            ...
            // Send AJAX request
            fetch(window.location.origin + '/Elite/' + controllerName + '/deleteProfileImage', {
                method: 'POST'
            })
            .then(response => response.json())
            .then(data => {
