...
<div class="<?php echo $layout; ?>">
    <div class="<?php echo $sidebar; ?>" id="staffSidebar">
        <div class="sidebar-header">
            <div class="<?php echo $isCoach ? 'coach-logo' : 'trainer-info'; ?>">
                <?php if ($isCoach): ?>
                    <i class="fas <?php echo $logo; ?>"></i><h3><?php echo $panelName; ?></h3>
                <?php else: ?>
    ...
    <main class="main-content" id="mainContent">
        ...
        <div style="padding:0 25px 40px;">
            ...
            <div style="display:flex;align-items:center;gap:16px;margin-bottom:16px;flex-wrap:wrap;">
                <a href="<?php echo URLROOT; ?>/staffslots/calendar/<?php echo htmlspecialchars($data['prevWeek'] ?? date('Y-m-d', strtotime('-7 days'))); ?>" style="padding:7px 16px;background:#ecf0f1;border-radius:6px;text-decoration:none;color:#333;font-size:13px;">
                    <i class="fas fa-chevron-left"></i> Prev Week
                </a>
                <span style="font-weight:700;font-size:15px;color:#2c3e50;">
                    <?php echo date('j M Y', strtotime($data['from'] ?? date('Y-m-d'))); ?> &mdash; <?php echo date('j M Y', strtotime($data['to'] ?? date('Y-m-d'))); ?>
                </span>
                <a href="<?php echo URLROOT; ?>/staffslots/calendar/<?php echo htmlspecialchars($data['nextWeek'] ?? date('Y-m-d', strtotime('+7 days'))); ?>" style="padding:7px 16px;background:#ecf0f1;border-radius:6px;text-decoration:none;color:#333;font-size:13px;">
                    Next Week <i class="fas fa-chevron-right"></i>
                </a>
                <a href="<?php echo URLROOT; ?>/staffslots/calendar" style="padding:7px 16px;background:#2e7d32;color:#fff;border-radius:6px;text-decoration:none;font-size:13px;margin-left:auto;">
                    Today
                </a>
            </div>
            ...
            <div style="display:flex;gap:16px;margin-bottom:20px;flex-wrap:wrap;">
                <div style="background:#fff;border-radius:10px;padding:14px 22px;box-shadow:0 1px 6px rgba(0,0,0,.07);min-width:140px;">
                    <div style="font-size:22px;font-weight:700;color:#2e7d32;"><?php echo $totalSessions; ?></div>
                    <div style="font-size:12px;color:#888;margin-top:2px;">Sessions this week</div>
                </div>
                <div style="background:#fff;border-radius:10px;padding:14px 22px;box-shadow:0 1px 6px rgba(0,0,0,.07);min-width:140px;">
                    <div style="font-size:22px;font-weight:700;color:#004085;"><?php echo $totalExpected; ?></div>
                ...
                <table class="cal-calendar-table">
                    <thead>
                        <tr>
                            ...
                            <?php for ($i = 0; $i < 7; $i++): ?>
                                <?php
                                $ts = strtotime("+{$i} days", (int)($data['monTs'] ?? time()));
                                $date = date('Y-m-d', $ts);
                                $isToday = ($date === date('Y-m-d'));
                                ?>
                                <th style="padding:10px 8px;text-align:center;font-size:13px;color:#555;border-bottom:2px solid #dee2e6;">
                                    <div style="font-weight:700;"><?php echo $dayNames[$i]; ?></div>
                                    <div style="font-size:12px;color:#888;"><?php echo date('j M', $ts); ?></div>
                                </th>
                            <?php endfor; ?>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <?php for ($i = 0; $i < 7; $i++): ?>
                                ...
                                ?>
                                <td class="cal-cell<?php echo $isToday ? ' cal-today' : ''; ?>">
                                    <?php if (empty($occs)): ?>
                                    ...
                                    <?php else: ?>
                                        <?php foreach ($occs as $occ): ?>
                                            ...
                                            ?>
                                            <a href="<?php echo URLROOT; ?>/staffslots/occurrence/<?php echo (int)$occ->OccurrenceID; ?>" class="cal-card <?php echo $cls; ?>">
                                                <div style="font-weight:700;margin-bottom:2px;">
                                                    <?php echo htmlspecialchars($occ->SessionName ?? 'Session'); ?>
                                                </div>
                                                <div><?php echo htmlspecialchars($occ->SlotLabel ?? '—'); ?></div>
                                                <?php if (!empty($occ->FacilityName)): ?>
                                                    <div style="opacity:.8;"><?php echo htmlspecialchars($occ->FacilityName); ?></div>
                                                <?php endif; ?>
